...
def get_parser():
    ...
    group_para.add_argument('--temporal_mask', action='store', type=op.abspath,
                            help='the path for the (temporal) mask file.\n'
                                 'The mask file should be a ".dat" file, consisting of a binary string '
                                 'of the same length as the signal')
    ...
    # ---- New Wiener knobs (feature parity with MATLAB v2.5 notes)
    group_para.add_argument('--iter-noise', action='store_true',
                            help='re-estimate noise at each Wiener iteration')

    group_para.add_argument('--post-smooth', type=float, default=0.0,
                            help='Gaussian sigma (samples) for final output; 0 disables')

    group_para.add_argument('--pad-factor', type=float, default=2.0,
                            help='reflect padding factor (e.g., 4.0–5.0 for strong edge control)')

    group_para.add_argument('--tukey-alpha', type=float, default=0.15,
                            help='Tukey edge taper alpha for Wiener (0..1)')

    group_para.add_argument('--tol', type=float, default=1e-6,
                            help='early-stop tolerance for Wiener (relative change)')
...
def run_rsHRF():
    ...
    # ---- collect Wiener options for pass-through
    wiener_opts = dict(
        iter_noise=para.get('iter_noise', False),
        post_smooth=para.get('post_smooth', 0.0),
        pad_factor=para.get('pad_factor', 2.0),
        tukey_alpha=para.get('tukey_alpha', 0.15),
        tol=para.get('tol', 1e-6),
    )
    ...
    if args.estimation in ('sFIR', 'FIR'):
        para['T'] = 1
    ...
    if args.ts is not None:
        ...
        fourD_rsHRF.demo_rsHRF(args.ts, None, args.output_dir, para, args.n_jobs, file_type,
                               mode='time-series', temporal_mask=temporal_mask, wiener=args.wiener,
                               wiener_opts=wiener_opts)

    # ---- single file mode
    if args.input_file is not None:
        if args.atlas is not None:
            if (args.input_file.endswith(('.nii', '.nii.gz')) and args.atlas.endswith(('.gii', '.gii.gz'))) or \
               (args.input_file.endswith(('.gii', '.gii.gz')) and args.atlas.endswith(('.nii', '.nii.gz'))):
        ...
        para['lag'] = np.arange(np.fix(para['min_onset_search'] / para['dt']),
                                np.fix(para['max_onset_search'] / para['dt']) + 1, dtype='int')
        fourD_rsHRF.demo_rsHRF(args.input_file, args.atlas, args.output_dir, para, args.n_jobs, file_type,
                               mode='input', temporal_mask=temporal_mask, wiener=args.wiener,
                               wiener_opts=wiener_opts)

    # ---- BIDS validation for derivatives
    ...
    if args.bids_dir is not None and args.atlas is not None:
        ...
        if args.bids_filter_file is not None:
            ...
            all_inputs = layout.get(return_type='filename', datatype='func',
                                    subject=subjects_to_analyze, task='rest', desc='preproc',
        ...
        if not all_inputs != []:
            ...
            for file_count in range(len(all_inputs)):
                ...
                num_errors += 1
                try:
                    fourD_rsHRF.demo_rsHRF(all_inputs[file_count], args.atlas, args.output_dir, para,
                                           args.n_jobs, file_type, mode='bids w/ atlas',
                                           temporal_mask=temporal_mask, wiener=args.wiener,
                                           wiener_opts=wiener_opts)
                    num_errors -= 1
    ...
    if args.bids_dir is not None and args.brainmask:
        ...
        if not subjects_to_analyze:
            parser.error('Could not find participants. Please make sure the BIDS data '
                         'structure is present and correct.')
        ...
        if args.bids_filter_file is not None:
            ...
            default_mask = {'extension': 'nii.gz', 'datatype': 'func', 'desc': 'brain',
                            'task': 'rest', 'suffix': 'mask'}
            ...
            all_inputs = layout.get(return_type='filename', datatype='func',
                                    subject=subjects_to_analyze, task='rest', desc='preproc',
                                    suffix='bold', extension=['nii', 'nii.gz'])
            all_masks = layout.get(return_type='filename', datatype='func',
                                   subject=subjects_to_analyze, desc='brain', task='rest',
        ...
        if not all_prefix_match:
            ...
            for file_count in range(len(all_inputs)):
                ...
                if file_type in (".nii", ".nii.gz"):
                    ...
                    TR = spm_dep.spm.spm_vol(all_inputs[file_count]).get_arrays_from_intent(
                        "NIFTI_INTENT_TIME_SERIES")[0].meta.get_metadata()["TimeStep"]
                ...
                num_errors += 1
                try:
                    fourD_rsHRF.demo_rsHRF(all_inputs[file_count], all_masks[file_count], args.output_dir,
                                           para, args.n_jobs, mode='bids',
                                           temporal_mask=temporal_mask, wiener=args.wiener,
                                           wiener_opts=wiener_opts)
                    num_errors -= 1
                except ValueError as err:
            ...
            success = len(all_inputs) - num_errors
            if success == 0:
                raise RuntimeError('Dimensions were inconsistent; no inputs were processed!')
