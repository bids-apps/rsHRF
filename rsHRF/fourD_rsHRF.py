...
def demo_rsHRF(
    ...
    if mode != "time-series":
        ...
        if mask_file != None:
            ...
            if (
                ...
                raise ValueError(
                    "Inconsistency in input-mask dimensions"
                    + "\n\tinput_file == "
                    + name
                    + file_type
                    + "\n\tmask_file == "
    ...
    def process_voxel_deconv(
        voxel_id, hrf_array, sig_deconv, nobs_val, p_wiener, p_para
    ):
    ...
    results = Parallel(n_jobs=p_jobs)(
        delayed(process_voxel_deconv)(
            v_id, hrfa_TR, bold_sig_deconv, nobs, wiener, para
        )
    ...
    plt.figure()
    plt.plot(
        para["TR"] * np.arange(1, np.amax(hrfa_TR[:, pos].shape) + 1),
        hrfa_TR[:, pos],
        linewidth=1,
    )
    plt.xlabel("time (s)")
    ...
    markerline, stemlines, baseline = plt.stem(
        para["TR"] * np.arange(1, nobs + 1), event_plot
    )
