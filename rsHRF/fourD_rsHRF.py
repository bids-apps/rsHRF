...
def demo_rsHRF(input_file, mask_file, output_dir, para, p_jobs, file_type=".nii",
               mode="bids", wiener=False, temporal_mask=[], wiener_opts=None):
    """
    Main entry point for rsHRF processing on time-series or 4D volumes.

    Parameters
    ----------
    input_file : str
        Path to BOLD file (.nii/.nii.gz or .gii/.gii.gz) or .txt (time-series).
    mask_file : str or None
        Path to mask/atlas file (same type as input). None -> inferred brain.
    output_dir : str
        Output directory.
    para : dict
        Parameter dict (TR, T, passband, etc.).
    p_jobs : int
        Parallel jobs (unused here, kept for compatibility).
    file_type : str
        '.nii' / '.nii.gz' / '.gii' / '.gii.gz' or tuple from splitext for --ts.
    mode : {'bids', 'bids w/ atlas', 'input', 'time-series'}
    wiener : bool
        If True, perform iterative Wiener deconvolution.
    temporal_mask : list[int]
        Optional time-point mask (0/1).
    wiener_opts : dict or None
        Extra knobs passed to iterative_wiener_deconv (feature parity with MATLAB notes).
    """

    # ---------------- book-keeping for localK ----------------
    ...
    if mode != 'time-series':
        ...
        if mask_file is not None:
            ...
            # dimension checks between input and mask
            if ((file_type in (".nii", ".nii.gz") and
                 v1.header.get_data_shape()[:-1] != v.header.get_data_shape()) or
                (file_type in (".gii", ".gii.gz") and
    ...
    # Prepare kwargs for Wiener (feature parity knobs)
    wkwargs = dict(Iterations=1000)
    if wiener_opts:
        wkwargs.update({k: v for k, v in wiener_opts.items() if v is not None})
    # You can also pass TR if your Wiener accepts it:
    # wkwargs.setdefault('TR', para.get('TR', None))
    ...
    for voxel_id in range(nvar):
        ...
        if not wiener:
            ...
            data_deconv[:, voxel_id] = iterative_wiener_deconv.rsHRF_iterative_wiener_deconv(
                bold_sig_deconv[:, voxel_id], hrf, **wkwargs
            )
    ...
    plt.plot(para['TR'] * np.arange(1, nobs + 1),
             np.nan_to_num(stats.zscore(bold_sig[:, pos], ddof=1)), linewidth=1)
    plt.plot(para['TR'] * np.arange(1, nobs + 1),
             np.nan_to_num(stats.zscore(data_deconv[:, pos], ddof=1)), color='r', linewidth=1)
