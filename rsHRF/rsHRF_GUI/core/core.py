...
class Core:
    ...
    def makeInput(self, inp):
        ...
        # if the input is present in a directory organized in BIDS format
            ...
            layout = BIDSLayout(
                input_file, validate=False, config=["bids", "derivatives"]
            )
            ...
            if not subjects_to_analyze:
                return Status(
                    False,
                    error="Could not find participants. Please make sure the BIDS data "
            ...
            # if the atlas file is not provided
                ...
                if not all_masks != []:
                    return Status(
                        False,
                        error="There are no files of type *mask.nii / *mask.nii.gz "
                        "Please make sure to have at least one file of the above type "
                        "in the BIDS specification",
                    )
                ...
                if len(all_inputs) != len(all_masks):
                    return Status(
                        False,
                        error="The number of *bold.nii / .nii.gz and the number of "
                ...
                if not all_prefix_match:
                    return Status(
                        False,
                        error="The mask and input files should have the same prefix for correspondence. "
    ...
    def makeInputFile(self, input_file, mask_file, file_type, mode):
        ...
        if (
            (file_type == ".nii" or file_type == ".nii.gz")
        ...
        if not subject.is_present("BOLD", input_file):
            ...
            bold_ts = BOLD_Raw(
                ...
                subject_index=subject.get_subject_index(),
            )
        ...
        if not subject.is_present(
            "Preprocessed-BOLD", (self.parameters, mask_file, bold_ts)
        ):
            ...
            bold_pre_ts = BOLD_Preprocessed(
                label="Preprocessed-BOLD",
                ts=np.array(bold_sig),
                para=self.parameters,
                subject_index=subject.get_subject_index(),
            )
            ...
            return Status(
                True,
                info="Preprocessed Input!",
                dic={
                    "Number of Time-Slices: ": bold_ts.get_shape()[0],
                    "Number of Brain-Voxels: ": bold_ts.get_shape()[1],
                },
            )
        return Status(
            False, error="Time series already exists. No new additions were made."
        )
            ...
            if get_pos:
                return (
                    Status(
                        True,
                        info="Retrieved HRF!",
                        dic={"Shape of HRF: ": hrf.get_shape()},
                    ),
                    pos,
                )
            else:
                return Status(
                    True, info="Retrieved HRF!", dic={"Shape of HRF: ": hrf.get_shape()}
                )
        if get_pos:
            return (
                Status(
                    False,
                    error="Time series already exists. No new additions were made.",
                ),
                None,
            )
        else:
    ...
    def getHRFParameters(self, hrf):
        ...
        if hrf.get_HRF_para().size != 0:
            return Status(
                False, error="Parameters have already been computed for this HRF"
            )
        ...
        return Status(
            True,
            info="Retrieved HRF Parameters",
            dic={
                "Average Height: ": str(average[0])[:3],
                "Avearage Time-To-Peak: ": str(average[1])[:3] + " seconds",
                "Avearge Full-Width at Half-Max: ": str(average[2])[:3],
            },
        )
    ...
    def deconvolveHRF(self, hrf):
        ...
        if not subject.is_present("Deconvolved-BOLD", (self.parameters, hrf)):
            ...
            for voxel_id in range(nvar):
                ...
                H = np.fft.fft(
                    np.append(hrf_, np.zeros((nobs - max(hrf_.shape), 1))), axis=0
                )
