...
def wgr_regress(y, X):
    ...
    if R.ndim == 0:
        ...
        p = int(
            np.abs(R[0]).item() > 0
        )  # Converting to scalar using .item() for compatibility and safe access
    else:
        if np.amin(R.shape) == 1:
            ...
            p = np.sum(
                np.abs(np.diagonal(R)) > abs(max(n, ncolX) * np.spacing(R[0][0]))
            )
    if p < ncolX:
        R = R[0:p, 0:p]
        Q = Q[:, 0:p]
...
def Fit_sFIR2(output, length, TR, input, T, flag_sfir, AR_lag):
    ...
    X = linalg.toeplitz(
        input.flatten(), np.concatenate((_input.flatten(), np.zeros(NN - 1)))
    )
    if X.ndim < 2:
...
def wgr_FIR_estimation_HRF(u, dat, para, N):
    ...
    for i_lag in range(1, nlag + 1):
        ...
        if RR.size != 0:
            ...
            design[RR] = 1
            hrf_kk, e3 = Fit_sFIR2(
                dat, para["len"], para["TR"], design, len_bin, firmode, para["AR_lag"]
            )
