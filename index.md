 ...
 * A BIDS formatted data-set directory. This option can be accessed using the ``--bids_dir`` optional argument followed by the path of the directory.
 ...
 Out of the above 3 options, one of them is always required and more than one cannot be supplied at one.
 ...
 * Out of the above 2 options, both cannot be supplied at once. In the case where neither of the 2 options are supplied, the app proceeds to generate a mask-file by computing the variance. However, providing a mask-file is strongly recommended.

* Also, ``--input_file`` and ``--brainmask`` together are an invalid combination.

 The other 5 use-cases are explained below:

 * ``--input_file`` : The standalone ``.nii / .nii.gz or .gii / .gii.gz`` file  is passed to the application for the analysis, the mask file gets generated by computing the variance, and the outputs are determined accordingly.

 * ``--ts`` : The standalone ``.txt`` file containing a time-series (floating point values by line breaks), or multiple time-series' (`,` seperated floating point values, where one time-series is represented by a single column). No mask file is required in this scenario, and the outputs are determined accordingly.
...
``rsHRF --ts input.txt --estimation gamma --output_dir results -TR 2.0``
...
``rsHRF --input_file input.nii --estimation canon2dd --output_dir results``.
...
In the above example, the input file is ```input.txt```. The estimation method passed is `fourer`. The `-TR` argument (which represents the BOLD repetition time) needs to be supplied if ```.gii / .gii.gz``` input file is used.
